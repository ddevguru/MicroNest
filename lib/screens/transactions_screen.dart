...
class _TransactionsScreenState extends State<TransactionsScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadTransactions() async {
    try {
      ...
      final groupResult = await GroupService.getUserGroups();
      if (groupResult['success'] == true) {
        // Extract transactions from user groups
        groupTransactions = [];
        final groups = groupResult['groups'] ?? [];
        for (var group in groups) {
          if (group['transactions'] != null) {
            groupTransactions.addAll(List<Map<String, dynamic>>.from(group['transactions']));
          }
        }
      }
