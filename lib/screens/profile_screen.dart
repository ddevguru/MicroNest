...
class _ProfileScreenState extends State<ProfileScreen> with TickerProviderStateMixin {
                        ...
                        Expanded(
                          child: Column(
                            children: [
                              ...
                              const SizedBox(height: 8),
                              GestureDetector(
                                onTap: () => _showImageSourceDialog(true, (path) {
                                  setState(() {
                                    frontImagePath = path;
                                  });
                                }),
                        ...
                        Expanded(
                          child: Column(
                            children: [
                              ...
                              const SizedBox(height: 8),
                              GestureDetector(
                                onTap: () => _showImageSourceDialog(false, (path) {
                                  setState(() {
                                    backImagePath = path;
                                  });
                                }),
  ...
  void _showImageSourceDialog(bool isFront, Function(String) onImagePicked) async {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          backgroundColor: const Color(0xFF1A1A1A),
          title: Text(
            'Select ${isFront ? 'Front' : 'Back'} Image',
            style: const TextStyle(color: Colors.white),
          ),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              ListTile(
                ...
                onTap: () {
                  Navigator.of(context).pop();
                  _pickImageFromGallery(isFront, onImagePicked);
                },
              ),
              ListTile(
                ...
                onTap: () {
                  Navigator.of(context).pop();
                  _takePhoto(isFront, onImagePicked);
                },
              ),
            ],
          ),
  ...
  Future<void> _pickImageFromGallery(bool isFront, Function(String) onImagePicked) async {
    try {
      ...
      if (!hasPermission) {
        if (mounted) {
          _showPermissionDialog('Photos', 'photo library');
        }
        return;
      }
      ...
      if (image != null) {
        ...
        // Check file size
        if (!ImageService.isFileSizeAcceptable(imageFile, maxSizeMB: 5.0)) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
  ...
  Future<bool> _checkAndRequestPermissions() async {
    ...
    if (!allGranted) {
      if (mounted) {
        _showDetailedPermissionDialog(statuses);
      }
    }
  ...
  void _clearImage(bool isFront) {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${isFront ? 'Front' : 'Back'} image cleared'),
          backgroundColor: Colors.orange,
        ),
      );
    }
  ...
  Future<void> _takePhoto(bool isFront, Function(String) onImagePicked) async {
    try {
      ...
      if (image != null) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('${isFront ? 'Front' : 'Back'} photo captured successfully!'),
              backgroundColor: const Color(0xFF52B788),
            ),
          );
        }
  ...
  void _showDetailedPermissionDialog(Map<String, PermissionStatus> statuses) {
    String content = 'Current permission statuses:\n\n';
    
    statuses.forEach((key, value) {
      content += '${key.toUpperCase()}: ${PermissionService.getPermissionStatusText(value)}\n';
    });
    
    content += '\nTo use image picker, please grant the required permissions in settings.';
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
            'This app needs $permissionType permission to access $feature. Please grant permission in settings.',
            style: const TextStyle(color: Colors.white70),
          ),
          actions: [
            TextButton(
  ...
  void _showPermissionStatus() async {
    ...
    String statusText = 'Current Permission Status:\n\n';
    statuses.forEach((key, value) {
      statusText += '${key.toUpperCase()}: ${PermissionService.getPermissionStatusText(value)}\n';
    });
